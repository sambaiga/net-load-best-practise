...
#https://github.com/AyrtonB/WPD-DS-Challenge/blob/main/batopt/clean.py
...
def pv_anomalies_to_nan(df_pv, model=GradientBoostingRegressor(), tolerance=0.1):
    ...
    X, y, dates = split_X_y_data_with_index(df_power_features, 'PV')
    ...
    df_pv.loc[df_pv.index.isin(anomalous_idx), 'PV'] = np.nan

    return df_pv

# Cell
def interpolate_missing_site_power(df_pv, model=RandomForestRegressor()):
    missing_site_power_dts = df_pv.index[df_pv['PV'].isnull()]

    if len(missing_site_power_dts) == 0: # i.e. no missing values
        return df_pv

    df_power_features = construct_df_power_features(df_pv)
    missing_dt_X = df_power_features.loc[missing_site_power_dts].drop('PV', axis=1).values
    X, y = split_X_y_data(df_power_features, 'PV')
